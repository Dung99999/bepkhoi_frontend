...
const RoomAreaManagePage: React.FC = () => {
    ...
    const handleAddRoomArea = async (values: { roomAreaName: string; roomAreaNote: string }) => {
        setLoading(true);
        try {
            const response = await fetch(`${process.env.REACT_APP_API_APP_ENDPOINT}api/roomarea/create`, {
                method: "POST",
                headers: {
    ...
    const handleUpdateRoomArea = async (values: { roomAreaName: string; roomAreaNote: string }) => {
