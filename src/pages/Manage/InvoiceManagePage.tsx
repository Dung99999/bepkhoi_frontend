...
import { Table, Empty, Tag, Collapse } from "antd";
import type { ColumnsType } from "antd/es/table";
import { CaretRightOutlined, FileTextOutlined, ShoppingOutlined, DollarOutlined } from "@ant-design/icons";
      ...
      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div className="bg-white p-3 rounded-lg shadow-sm">
            <h3 className="font-semibold text-gray-700 mb-2">Thông tin hóa đơn</h3>
        ...
        <h3 className="font-semibold text-gray-700 mb-3">Chi tiết đơn hàng</h3>
        
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-100">
              <tr>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Món</th>
                <th className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">SL</th>
                <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Đơn giá</th>
                <th className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">VAT</th>
              ...
              {record.invoiceDetails.map((item) => (
                <tr key={item.invoiceDetailId} className="hover:bg-blue-50">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900 text-right">
                    {Math.round(item.price * item.quantity * (1 + item.productVat/100)).toLocaleString('vi-VN')}₫
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                    {item.productNote || <span className="text-gray-400">Không có</span>}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <Table
          ...
          expandable={{
            ...
            expandIcon: ({ expanded, onExpand, record }) => (
              <CaretRightOutlined
                className={`transition-transform duration-200 ${expanded ? 'transform rotate-90' : ''} text-blue-500`}
